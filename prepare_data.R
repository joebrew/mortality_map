...
if('all_data.RData' %in% dir('data')){
  ...
  # Get population and deaths per zone per year BY SEX
  pop_sex <-
    member_expanded %>%
    filter(years_of_age >= 18,
           years_of_age <= 50) %>%
  ...
  mort_two <-
    ...
    rename(n_deaths = deaths) %>%
    mutate(p_deaths = n_deaths / pop) %>%
    mutate(age_group = '30-50')
  mortality <-
    bind_rows(mortality,
              mort_two)
  
  # Create sex-specific mortality too
  mortality_sex <-
    member_expanded %>%
    filter(years_of_age >= 30,
           years_of_age <= 50) %>%
  ...
  # Define function for getting file numbers
  file_number <- 
    function () {
      if(!exists('o')){
        o <<- 0
  ...
  # Define function for joining mortality to zonas shp
  join_data_sex <- function(year = 2010,
                        age_group = '30-50',
                        sex = 'M'){
    the_year <- year
    the_age_group <- age_group
    sub_mortality <- mortality_sex %>%
      filter(year == the_year,
             gender == sex)
    sub_mortality <- sub_mortality %>%
      filter(age_group == the_age_group)
...
theme_mortality <-
  function (base_size = 14, base_family = "Helvetica", subtitle_family = "Helvetica", 
    ...
    theme_bw(base_size = base_size1) + theme(panel.background = element_rect(fill = color_background, 
